...
#include "GifAnimViewer.h"
    ...
    uint32_t lastMasterBootToken = 0;   // Tracked by Slave to detect Master restarts
    IPAddress dynamicMasterIP(192, 168, 4, 1); // Dynamically updated by Slave
    ...
    void send_slave_ip_to_master() {
        ...
        if (WAVESHARE_WIFI_CLIENT_MODE && WiFi.status() == WL_CONNECTED && !slave_ip_sent) {
            ...
            if (client.connect(dynamicMasterIP, 23)) {
    ...
    void listen_for_master_broadcast() {
        if (WAVESHARE_WIFI_CLIENT_MODE && WiFi.status() == WL_CONNECTED) {
            ...
            if (packetSize >= sizeof(uint32_t)) {
                ...
                if (senderIP != dynamicMasterIP || receivedToken != lastMasterBootToken) {
                    Serial.printf("Master Found! IP: %s, Token: %u\n", senderIP.toString().c_str(), receivedToken);
                    dynamicMasterIP = senderIP;
...
void connectionStatus(const char *ssid, bool trying, bool failure, bool success)
{
  ...
  if (!ap and failure_cnt > 0) {
      if (WAVESHARE_WIFI_CLIENT_MODE and failure_cnt > 1) {

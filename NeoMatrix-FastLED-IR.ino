...
#include "GifAnimViewer.h"
    ...
    IPAddress Master_IP(192, 168, 4, 1); // Dynamically updated by Slave
    ...
    void send_slave_ip_to_master() {
        ...
        if (WIFI_CLIENT_MODE && WiFi.status() == WL_CONNECTED && !slave_ip_sent) {
            ...
            if (client.connect(Master_IP, 23)) {
                ...
                client.clear(); 
                // Give chance for IP to get through before closing connection
                delay(500);
                client.stop();
                slave_ip_sent = true;
    ...
    void listen_for_master_broadcast() {
        if (WIFI_CLIENT_MODE && WiFi.status() == WL_CONNECTED) {
...
void loop() {
	...
	EVERY_N_SECONDS(60) {
          MS_Print(); Serial.printf("Loop() - Free Stack Space: %d\r\n", uxTaskGetStackHighWaterMark(NULL));
	}
    #ifdef WIFI
        ArduinoOTA.handle();
        Splitter.handle(); 
